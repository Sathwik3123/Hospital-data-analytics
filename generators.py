...
def random_datetime_between(start_date, end_date):
    ...
    # ✅ SAFETY CHECK - Prevents errors when start >= end
...
def gen_specialization(i, department_id):
...
# DOCTOR
# =========================
def gen_doctor(i, department_id, specialization_id, hospital_branch_id):
    ...
    return {
        ...
        "is_active": True,
        "is_hod": False,  # Will be updated later for one doctor per department
        "created_date": now,
        "created_by": SYSTEM_USER,
        "updated_date": None,
        "updated_by": None,
...
def gen_treatment(i, department_id):
...
def gen_appointment(i, patient_id, doctor_id, hospital_branch_id, patient_created_date, start_date, end_date):
    """Generate appointment record with proper chronology"""
...
def gen_patient_visit(
    ...
    # ✅ ENHANCED: Convert datetime to date for admission
...
def gen_billing(i, patient_visit_id, hospital_branch_id, billing_type_id):
    ...
    # ✅ ENHANCED: More realistic amount ranges by billing type
    amount_ranges = {
        1: (500, 5000),       # Pharmacy
...
def gen_pharmacy_item(i):
    ...
    # Add slight price variation (±10 to ±20)
...
def gen_radiology_billing_detail(i, billing_id, item_id):
    ...
    # Usually quantity is 1 for radiology services (90% chance)

...
def main():
    ...
    specializations = []
    for _ in range(CONFIG["specializations"]):
        specializations.append(gen_specialization(spec_id))
        registry.register("specialization", spec_id)
        spec_id += 1
    write_excel("specialization", specializations, folder)
    ...
    for _ in range(CONFIG["doctors"]):
        doctors.append(
            gen_doctor(
                doc_id,
                registry.get("department"),
                registry.get("specialization")
            )
        )
    ...
    treatments = []
    for _ in range(CONFIG["treatments"]):
        treatments.append(gen_treatment(treatment_id))
    ...
    for patient_info in patient_data:
        ...
        for visit_num in range(num_visits):
            ...
            # Create visit
            treatment_id = registry.get("treatment") if random.random() > 0.3 else None

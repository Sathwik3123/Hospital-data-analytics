...
def load_payment_type():
    """
    ETL process to load payment type data from OLTP into OLAP 'dim_payment_type' dimension table.
    ...
    # 1. Setup
    # ----------------------------
    file_name = os.path.basename(__file__)  # e.g., "dim_payment_type.py"
    ...
    try:
        # ----------------------------
        # 2. EXTRACT: fetch payment types
        # ----------------------------
        oc.execute("SELECT id, payment_type_name FROM payment_type")
        rows = oc.fetchall()

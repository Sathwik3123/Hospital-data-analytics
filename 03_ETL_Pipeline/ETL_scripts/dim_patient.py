...
def load_patient():
    """
    ETL process to load patient data from OLTP into OLAP 'dim_patient' dimension table.
    ...
    # 1. Setup
    # ----------------------------
    file_name = os.path.basename(__file__)  # e.g., "dim_patient.py"
    ...
    try:
        # ----------------------------
        # 2. EXTRACT: fetch patient data
        # ----------------------------
        oc.execute("SELECT id, first_name, dob, gender,phone,email,address FROM patient")
        rows = oc.fetchall()
        ...
        for r in rows:
            try:
                ac.execute("""
                    ...
                    (patient_id, first_name, date_of_birth, gender,phone,email,address) 
                    VALUES (?, ?, ?, ?,?,?,?)
                """, r)
